 ...
 * sensor a metric for the average, maximum, or other statistics computed off the sequence of
 * message sizes that are recorded by the sensor. <p> Usage looks something like this:
 *
 * <pre>
 * // set up metrics:
...
public class Metrics {
  ...
  /**
   * Create a metrics repository with no reporters and the given default config. This config will be
   * used for any metric that doesn't override its own config.
   *
   * @param defaultConfig The default config to use for all metrics that don't override their
   *                      config
   */
   ...
   *
   * @param name    The name of the sensor
   * @param config  A default configuration to use for this sensor for metrics that don't have their
   *                own config
   * @param parents The parent sensors
   * @return The sensor that is created
   */
  ...
  /**
   * Add a metric to monitor an object that implements measurable. This metric won't be associated
   * with any sensor. This is a way to expose existing values as metrics.
   *
   ...
   * @param measurable  The measurable that will be measured by this metric
   */
  public synchronized void addMetric(MetricName metricName, MetricConfig config, Measurable
      measurable) {
